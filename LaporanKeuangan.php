            ...
            <div id="main">
                ...
                <nav class="navbar" style="background-color: #1995ad;">
                    <div class="container-fluid">
                        <a id="header" class="navbar-brand" href=""><b>LAPORAN KEUANGAN</b></a>
                    </div>
                </nav>
                <br><br>
...
<option value="Semua">Semua</option>
         ...
         <option value="31">31</option>
                ...
                <!-- <form  action="cetakExcelLaporan.php"> -->
                <input id="tombolUnduhAll" class="btn btn-primary" data-toggle="modal" data-target="#exampleModalLong" type="submit" onclick="unduh()" value="Unduh File">
                ...
                <script>
                    function unduh() {
                        swal.fire({
                            ...
                            .then((result) => {
                                if (result.isConfirmed) {
                                    window.location = "cetakExcelLaporan.php";
                                }
                            });
                    }
                </script>
                    ...
                    if (isset($_POST['cari'])) {
                        ...
                        if ($hari == 'Semua' && $bulan == 'Semua' && $tahun == 'Semua') {
                            $sqlAll = "SELECT transaksi_jual.ID_Jual, transaksi_jual.Tanggal_Jual, detail_transaksi_jual.Kode_Barang, 
                                   detail_transaksi_jual.Harga_Jual_Barang, detail_transaksi_jual.Jumlah_Barang FROM transaksi_jual INNER JOIN detail_transaksi_jual ON transaksi_jual.ID_Jual = 
                                   detail_transaksi_jual.ID_Jual";
                            $queryAll = mysqli_query($conn, $sqlAll);
                            $hasilAll = [];
                            while ($temp = mysqli_fetch_assoc($queryAll)) {
                                $hasilAll[] = $temp;
                                $totalHarga[] = $temp['Harga_Jual_Barang'];
                            }
                            ...
                            echo "<td><b>Harga Jual<b></td>";
                            echo "<td><b>Jumlah Pembelian<b></td>";
                            echo "<td><b>Sub Total<b></td>";
                            echo "</tr>";
                            $totalDapat = 0;
                            $SubTotal = [];
                            foreach ($hasilAll as $data) {
                                $tempSubTotal = $data['Jumlah_Barang'] * $data['Harga_Jual_Barang'];
                                $SubTotal[] =  $tempSubTotal;
                                $totalDapat += $tempSubTotal;
                                echo "<tr id=tabelIsi align='center'>";
                                echo "<td>" . $data['ID_Jual'] . "</td>";
                                echo "<td>" . $data['Tanggal_Jual'] . "</td>";
                                echo "<td>" . $data['Kode_Barang'] . "</td>";
                                echo "<td>" . rupiah($data['Harga_Jual_Barang']) . "</td>";
                                echo "<td>" . $data['Jumlah_Barang'] . "</td>";
                                echo "<td>" . $tempSubTotal . "</td>";
                                echo "</tr>";
                            
                            }
                            $_SESSION['subTotal'] = $SubTotal;
                            $_SESSION['Total'] =  $totalDapat;
                            echo "<tr id = totalPendapatan>";
                            echo "<td colspan = '5' align = 'center'><b>TOTAL PENDAPATAN<b></td>";
                            echo "<td align = 'center'><b>Rp " . $totalDapat . "<b></td>";
                            echo "</tr>";
                            echo "</table>";
                        } else {
                            if ($bulan == 'Semua' && $tahun == 'Semua') {
                                ...
                                detail_transaksi_jual.Harga_Jual_Barang, detail_transaksi_jual.Jumlah_Barang FROM transaksi_jual INNER JOIN detail_transaksi_jual ON transaksi_jual.ID_Jual = 
                                detail_transaksi_jual.ID_Jual WHERE DAY(Tanggal_Jual) = $hari";
                            } else if ($hari == 'Semua' && $tahun == 'Semua') {
                                $sqlPilih = "SELECT transaksi_jual.ID_Jual, transaksi_jual.Tanggal_Jual, detail_transaksi_jual.Kode_Barang, 
                                detail_transaksi_jual.Harga_Jual_Barang, detail_transaksi_jual.Jumlah_Barang FROM transaksi_jual INNER JOIN detail_transaksi_jual ON transaksi_jual.ID_Jual = 
                                detail_transaksi_jual.ID_Jual WHERE MONTH(Tanggal_Jual) = $bulan";
                            } else if ($hari == 'Semua' && $bulan == 'Semua') {
                                $sqlPilih = "SELECT transaksi_jual.ID_Jual, transaksi_jual.Tanggal_Jual, detail_transaksi_jual.Kode_Barang, 
                                detail_transaksi_jual.Harga_Jual_Barang, detail_transaksi_jual.Jumlah_Barang FROM transaksi_jual INNER JOIN detail_transaksi_jual ON transaksi_jual.ID_Jual = 
                                detail_transaksi_jual.ID_Jual WHERE YEAR(Tanggal_Jual) = $tahun";
                            } else if ($hari == 'Semua') {
                                $sqlPilih = "SELECT transaksi_jual.ID_Jual, transaksi_jual.Tanggal_Jual, detail_transaksi_jual.Kode_Barang, 
                                detail_transaksi_jual.Harga_Jual_Barang, detail_transaksi_jual.Jumlah_Barang FROM transaksi_jual INNER JOIN detail_transaksi_jual ON transaksi_jual.ID_Jual = 
                                detail_transaksi_jual.ID_Jual WHERE MONTH(Tanggal_Jual) = $bulan AND YEAR(Tanggal_Jual) = $tahun";
                            } else if ($bulan == 'Semua') {
                                $sqlPilih = "SELECT transaksi_jual.ID_Jual, transaksi_jual.Tanggal_Jual, detail_transaksi_jual.Kode_Barang, 
                                detail_transaksi_jual.Harga_Jual_Barang, detail_transaksi_jual.Jumlah_Barang FROM transaksi_jual INNER JOIN detail_transaksi_jual ON transaksi_jual.ID_Jual = 
                                detail_transaksi_jual.ID_Jual WHERE YEAR(Tanggal_Jual) = $tahun AND DAY(Tanggal_Jual) = $hari";
                            } else if ($tahun == 'Semua') {
                                $sqlPilih = "SELECT transaksi_jual.ID_Jual, transaksi_jual.Tanggal_Jual, detail_transaksi_jual.Kode_Barang, 
                                detail_transaksi_jual.Harga_Jual_Barang, detail_transaksi_jual.Jumlah_Barang FROM transaksi_jual INNER JOIN detail_transaksi_jual ON transaksi_jual.ID_Jual = 
                                detail_transaksi_jual.ID_Jual WHERE MONTH(Tanggal_Jual) = $bulan AND DAY(Tanggal_Jual) = $hari";
                            } else {
                                $date = "$tahun-$bulan-$hari";
                                $sqlPilih = "SELECT transaksi_jual.ID_Jual, transaksi_jual.Tanggal_Jual, detail_transaksi_jual.Kode_Barang, 
                                detail_transaksi_jual.Harga_Jual_Barang, detail_transaksi_jual.Jumlah_Barang FROM transaksi_jual INNER JOIN detail_transaksi_jual ON transaksi_jual.ID_Jual = 
                                detail_transaksi_jual.ID_Jual WHERE Tanggal_Jual = '$date'";
                            }
                            ...
                            if (mysqli_num_rows($queryPilih) > 0) {
                                ...
                                $_SESSION['subTotal'] = $SubTotal;
                                $_SESSION['Total'] =  rupiah($totalDapat);
                                echo "<tr id = totalPendapatan>";
                                echo "<td colspan = '5' align = 'center'><b>TOTAL PENDAPATAN<b></td>";
                                echo "<td align = 'center'><b>Rp " .  rupiah($totalDapat) . "<b></td>";
                                echo "</tr>";
                                echo "</table>";
                            } else {
                                echo '<script>
                        ...
                        foreach ($hasilAll as $data) {
                            ...
                            echo "<td>" . rupiah($data['Harga_Jual_Barang']) . "</td>";
                            echo "<td>" . $data['Jumlah_Barang'] . "</td>";
                            echo "<td>" .  rupiah($tempSubTotal) . "</td>";
                            echo "</tr>";
                        }
                        $_SESSION['subTotal'] =  $SubTotal;
                        $_SESSION['Total'] =  rupiah($totalDapat);
